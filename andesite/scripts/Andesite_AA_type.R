#___TOPICOS___Andesite_ANDEAN_ARC___#

setwd("E:/Documentos/R/Topico_R/andesite/data") #Elija su directorio

library("readxl")
Andesite_AA <- read_excel("Andesite_AA.xlsx", col_types = c("text", "text", "text", "text", #1-4
...
#Andesite_AA1 <- Andesite_AA[c("TECTONIC SETTING","LOCATION","ROCK NAME","GEOL.","AGE","ROCK TEXTURE","ALTERATION","MINERAL","CR(PPM)", "CO(PPM)", "NI(PPM)", "CU(PPM)", "ZN(PPM)", "AS(PPM)", "SE(PPM)", "MO(PPM)", "CD(PPM)", "SB(PPM)", "HG(PPM)", "TL(PPM)", "PB(PPM)", "BI(PPM)")]
Andesite_AA1 <- Andesite_AA[c("TECTONIC SETTING","LOCATION","ROCK NAME","GEOL.","AGE","ROCK TEXTURE","ALTERATION","MINERAL")]
...
#Cr cromo
CR_PPM<-as.numeric(Andesite_AA$"CR(PPM)")
CR_PPM[is.na(CR_PPM)]<-0
hist(log10(CR_PPM))
#qqnorm(log(CR_PPM))

#Co cobalto
CO_PPM<-as.numeric(Andesite_AA$"CO(PPM)")
CO_PPM[is.na(CO_PPM)]<-0
hist(log10(CO_PPM))
#qqnorm(log10(CO_PPM))

#Ni niquel
NI_PPM<-as.numeric(Andesite_AA$"NI(PPM)")
NI_PPM[is.na(NI_PPM)]<-0
hist(log10(NI_PPM))
#qqnorm(log10(NI_PPM))

#Cu cobre
CU_PPM<-as.numeric(Andesite_AA$"CU(PPM)")
CU_PPM[is.na(CU_PPM)]<-0
hist(log10(CU_PPM))
#qqnorm(log10(CU_PPM))

#Zn zinc
ZN_PPM<-as.numeric(Andesite_AA$"ZN(PPM)")
ZN_PPM[is.na(ZN_PPM)]<-0
hist(log10(ZN_PPM))
#qqnorm(log10(ZN_PPM))

#As arsenico
AS_PPM<-as.numeric(Andesite_AA$"AS(PPM)")
AS_PPM[is.na(AS_PPM)]<-0
hist(log10(AS_PPM))
#qqnorm(log10(AS_PPM))

#Se selenio
SE_PPM<-as.numeric(Andesite_AA$"SE(PPM)")
SE_PPM[is.na(SE_PPM)]<-0
hist(log10(SE_PPM))
#qqnorm(log10(SE_PPM))

#Mo molibdeno
MO_PPM<-as.numeric(Andesite_AA$"MO(PPM)")
MO_PPM[is.na(MO_PPM)]<-0
hist(log10(MO_PPM))
#qqnorm(log10(MO_PPM))

#Cd cadmio
CD_PPM<-as.numeric(Andesite_AA$"CD(PPM)")
CD_PPM[is.na(CD_PPM)]<-0
hist(log10(CD_PPM))
#qqnorm(log10(CD_PPM))

#Sb antimonio
SB_PPM<-as.numeric(Andesite_AA$"SB(PPM)")
SB_PPM[is.na(SB_PPM)]<-0
hist(log10(SB_PPM))
#qqnorm(log10(SB_PPM))

#Hg mercurio
HG_PPM<-as.numeric(Andesite_AA$"HG(PPM)")
HG_PPM[is.na(HG_PPM)]<-0
hist(log10(HG_PPM))
#qqnorm(log10(HG_PPM))

#Tl talio
TL_PPM<-as.numeric(Andesite_AA$"TL(PPM)")
...
#Pb plomo
PB_PPM<-as.numeric(Andesite_AA$"PB(PPM)")
...
#Bi bismuto
BI_PPM<-as.numeric(Andesite_AA$"BI(PPM)")
...
#Sr estroncio
SR_PPM<-as.numeric(Andesite_AA$"SR(PPM)")
SR_PPM[is.na(SR_PPM)]<-0
hist(log10(SR_PPM))
#qqnorm(log10(SR_PPM))

#Y ytrio
Y_PPM<-as.numeric(Andesite_AA$"Y(PPM)")
Y_PPM[is.na(Y_PPM)]<-0
hist(log10(Y_PPM))
#qqnorm(log10(Y_PPM))
...
Andesite_AA1$Cr<-CR_PPM
Andesite_AA1$Co<-CO_PPM
Andesite_AA1$Ni<-NI_PPM
Andesite_AA1$Cu<-CU_PPM
Andesite_AA1$Zn<-ZN_PPM
Andesite_AA1$As<-AS_PPM
Andesite_AA1$Se<-SE_PPM
Andesite_AA1$Mo<-MO_PPM
Andesite_AA1$Cd<-CD_PPM
Andesite_AA1$Sb<-SB_PPM
Andesite_AA1$Hg<-HG_PPM
Andesite_AA1$Tl<-TL_PPM
Andesite_AA1$Pb<-PB_PPM
Andesite_AA1$Bi<-BI_PPM


library("corrplot")
cor.mat <- round(cor(Andesite_AA1[,c(9:20)]),2)
corrplot(cor.mat, type="upper", order="original", tl.col="black", tl.srt=45)

Andesite_AA.pca <- prcomp(Andesite_AA1[,c(9:20)], center = TRUE, scale. = TRUE )
plot(Andesite_AA.pca, choix = "varcor")
Andesite_AA.pca
summary(Andesite_AA.pca)

library("FactoMineR")
Andesite_AA.pca <- PCA(Andesite_AA1[,c(9:20)])  
