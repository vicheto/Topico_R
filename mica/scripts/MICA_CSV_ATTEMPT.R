...
Mica <- read.csv("E:\\Documentos\\R\\Topico_R\\mica\\data\\MICA.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE)
...
cor.mat <- round(cor(MicaA[,c(6:71)]),2)
corrplot(cor.mat, type="upper", order="original", tl.col="black", tl.srt=45)

Mica.pca <- prcomp(MicaA[,c(6:71)], center = TRUE, scale. = TRUE)
...
res.pca <- PCA(MicaA[,c(6:71)], ncp = 6, graph = FALSE)
