...
#MicaA.cleaned <- na.omit(MicaA)

#Limpieza modo II
#data.cleaned.2 <- data[complete.cases(data),]
#MicaA.cleaned2 <- data[complete.cases(MicaA),]

#G R A F I C O S 
library("corrplot")

cor.mat <- round(cor(MicaA[,c(2:67)]),2)
corrplot(cor.mat, type="upper", order="original", tl.col="black", tl.srt=45)

Mica.pca <- prcomp(MicaA[,c(2:67)], center = TRUE, scale. = TRUE)
